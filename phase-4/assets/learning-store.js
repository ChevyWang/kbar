...
function install(){if(document.getElementById('kbar-legacy-tools'))return;const root=document.createElement('section');root.id='kbar-legacy-tools';root.style.cssText='margin:2rem auto;padding:1rem;max-width:900px;border:1px solid #aaa;font:15px/1.7 sans-serif';root.innerHTML='<h2>专题任务记录</h2><p id="kbar-legacy-status" role="status"></p><button type="button" id="kbar-legacy-export">导出本页任务记录（含未提交草稿）</button> <label>导入任务记录 <input id="kbar-legacy-import" type="file" accept="application/json"></label><p>专题任务与结业研究的作答在此留痕。导入不覆盖不同内容的原答，冲突另存副本可再次导出。全部学习记录的完整备份，在<a href="https://chevywang.github.io/kbar/progress.html#backup">学习进度总览</a>页一次完成。</p>';document.body.appendChild(root);notice();
