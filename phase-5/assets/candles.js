...
(function () {
  ...
  function schematic(spec) {
    ...
    var schematicAria = bars.map(function (k, i) { return '第 ' + (i + 1) + ' 根：开 ' + k.o + '，高 ' + k.h + '，低 ' + k.l + '，收 ' + k.c; }).join('；');
    var annAria = (spec.ann || []).map(function (a) { return a.text || a.t; }).filter(Boolean).join('、');
    var s = '<svg data-kbar-chart="1" width="' + w + '" height="' + h + '" viewBox="0 0 ' + w + ' ' + h + '" role="img" aria-label="示意K线图：' + esc(schematicAria) + (annAria ? '；注释：' + esc(annAria) : '') + '。" style="max-width:100%">';
