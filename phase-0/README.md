...
课件内不含任何学习者个人信息——进度记录、学习记录、教练笔记请在课件包之外单独维护（在课件包之外单独维护，不随课件分发）。
...
[阶段实操与考核](mastery.html)提供本阶段教学卡与五份起步任务。结构化检查逐项通过，开放自评、AI反馈和专题任务分别记录；课内选择题分数不授予里程碑。任务数量和跨日间隔是待试学校准的设计参数。内容分钟是标称预算，不是掌握实测。
