  ...
  function mountBackup(target) {
    ...
    if (document.head && !document.getElementById('kbar-kbtn-style')) {
      ...
      kb.textContent = '.kbtn{font-family:var(--sans,system-ui,sans-serif);font-size:.85rem;border:1.5px solid var(--ink,#1c1c1a);border-radius:8px;padding:.32rem .85rem;margin:.2rem .45rem .2rem 0;cursor:pointer;background:#fff;color:var(--ink,#1c1c1a);min-height:36px}'
        ...
        + '.quiz-backup summary{cursor:pointer;min-height:36px;display:flex;align-items:center;font-family:var(--sans,system-ui,sans-serif);font-size:.88rem}'
    ...
    section.innerHTML = '<summary>备份训练场记录</summary><p>只含本课训练场的作答与复习；全部学习记录的备份与恢复，统一在<a href="https://chevywang.github.io/kbar/progress.html#backup">学习进度总览</a>页。</p><button id="quiz-export" type="button" class="kbtn">导出训练场记录</button><label class="kbtn kbtn-file">导入训练场记录<input id="quiz-import" type="file" accept="application/json"></label><p id="quiz-backup-status" role="status">冲突时整份停止，原答不覆盖；请保留两份备份文件。</p>';
